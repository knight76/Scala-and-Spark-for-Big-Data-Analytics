...
object KMeansDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // K-평균 모델을 사용해 데이터를 클러스터링한다.
    val numClusters = 5
    val numIterations = 20
    val run = 10
    val model = KMeans.train(landRDD, numClusters, numIterations, run, KMeans.K_MEANS_PARALLEL)

//    val numCluster = 5
//    val numIteration = 20
//    val seed = 12345
//    val model = KMeans.train(landRDD, numCluster, numIteration, seed)
